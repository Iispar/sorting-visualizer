...
const bubbleSort = (arr) => {
        ...
        arr.map(() => {
            for (var a = 0; a < (arr.length - 1); a++){
                ...
                // if the array is sorted halt further passes
                if (ready) {break}
            }
        })
    ...
    const selectionSort = (arr) => {
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            var smallestIndex = sliced.indexOf((Math.min(...sliced))) + i;
    ...
    // time(bubbleSort, arr, 'Bubble sort')
    // time(selectionSort, arr, 'Selection sort')
    // time(quickSort, arr, 'Quick Sort')
    // time(mergeSort, arr, 'Merge sort')


export {bubbleSort, selectionSort, quickSort, mergeSort}